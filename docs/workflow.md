...
具体真实传承人采访、真实口述、真实演出实录、真实仪式记录必须 requires_real_footage = true，不生成提示词。
空镜、工艺特写、材料特写、剪影、意象镜头、非特定人物/表演者/历史复原人物可由 AI 生成。
...
当 brief 或 FactCard 命中“真实传承人采访 / 口述史 / 现场演出 / 真实演出 / 授权档案 / 实录”等信号时，只把高风险人物槽位转换为：
...
这样既保留 5 镜结构，又不会让 AI 把真实采访、口述或演出实录伪装成生成内容。
...
禁止:
- 伪造具体真实传承人身份、真实采访、真实口述
- 把 AI 生成的人物说成真实人物或授权档案
- 塑料质感、过度干净的环境
- 商业广告风、奇幻特效
- 错误器具、错误工艺步骤
- 把 AI 镜头冒充真实历史影像
...
```text
纪录片分镜参考图,{shot_type}, 主体: {subject}; 构图: {composition}; 光线: {lighting}; 真实材质,低饱和,自然光,人物可为非特定 AI 生成角色,不得冒充真实传承人。
```
...
固定红线词扫描
工艺步骤无 fact_refs 检查
AI 镜头是否误触真实传承人身份伪造、真实档案冒充、广告化等规则
```
...
川剧变脸的真拍镜头来自 `portrait_interview` 转换，符合“真实采访、口述、现场演出实录不用 AI 伪造”的纪录片边界。
